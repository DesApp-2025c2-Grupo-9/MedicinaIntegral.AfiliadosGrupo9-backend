...
const seed = async () => {
  try {
    ...
    const prestadores = JSON.parse(
      fs.readFileSync(prestadoresFilePath, "utf-8")
    );
    ...
    const autorizacionesFilePath = path.resolve(
      "./src/json/autorizaciones.json"
    );
    const autorizaciones = JSON.parse(
      fs.readFileSync(autorizacionesFilePath, "utf-8")
    );
